...
def _patched_torch_load(f, map_location=None, pickle_module=None, weights_only=None, **kwargs):
    """
    Patched torch.load that handles the RNG state loading issue.
    Falls back to weights_only=False when weights_only=True fails.
    """
    
    # First, try with the original weights_only setting
    try:
        ...
        # If it failed and was related to weights_only, try again with weights_only=False
        if weights_only is True and ("UnpicklingError" in str(e) or "numpy" in str(e) or "_reconstruct" in str(e)):
            print(f"Warning: torch.load failed with weights_only=True, retrying with weights_only=False for file: {f}")
            return _original_torch_load(
                f, 
                map_location=map_location, 
                pickle_module=pickle_module, 
                weights_only=False,  # Force False on retry
                **kwargs
            )
        else:
            # Re-raise the original exception if it's not weights_only related
            raise e
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        modality_name2id = {'T2I':1, 'Caption':2, 'Text':3}
        dataset_name2id = {'dclm':1, 'laion-aesthetics':2, 'midjourney':3, 'blip3o-60k-short':4}
        processed_instances = []
        for sources in instances:
            if 'source' in sources.keys() and sources['source'] != None:
                source_info = json.loads(sources['source'])
                dataset_id = dataset_name2id[source_info['images']]
            else:
                dataset_id = 1
            if sources['data_type']  in ['image_text'] :
                ...
                if np.random.rand() < T2I_ratio : # T2I mode
                    ...
                    modality_id = 2
                    caption = sources['text']+self.tokenizer.eos_token
                ...
                dataset_ids = torch.ones_like(targets) * dataset_id
                modality_ids[: instruction_len] += 3
                if not self.data_args.ntp:
                    targets[: instruction_len] = IGNORE_INDEX          # NOTE: conditional generation
                ...
                text =  sources['text']
                assert  text != 'no'
                text = text + self.tokenizer.eos_token    # NOTE: this is not in the original code for unknown reason
        ...
        modality_ids = torch.nn.utils.rnn.pad_sequence(   # default padding side is right
            modality_ids,
            batch_first=True,
            padding_value=IGNORE_INDEX)
        ...
        for i, attention_mask in enumerate(attention_masks):
            false_idx = torch.where(~attention_mask)[0]
            if false_idx.numel() > 0:
                attention_masks[i][false_idx[0]] = True
        batch = dict(
            input_ids=input_ids,
            labels=labels,
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    if '^^' in data_path: # use ^^ to concat multi datasets
        ...
        for i,(percent,hgdata_path) in enumerate(zip(percentages,data_paths)):
...
def train(attn_implementation=None):
    ...
    if training_args.from_scratch:
        # Load the configuration (architecture) but not the weights
        config = AutoConfig.from_pretrained(
            model_args.model_name_or_path,
            cache_dir=training_args.cache_dir,
        )

        # Initialize model with random weights from the config
