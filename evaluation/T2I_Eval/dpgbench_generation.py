...
def main(args):
    ...
    prompt_files = glob.glob('../../../dpg_bench/prompts/*.txt')
    prompt_files.sort()
    ...
    for prompt_file in prompt_files:
        with open(prompt_file, 'r') as f:
            line = f.read()
        all_prompts.append({'Index': prompt_file.split('/')[-1].replace('.txt', ''),  'Prompt':line.strip()}) 
        
    chunked_filenames = np.array_split(all_prompts, num_chunks)
    subset = chunked_filenames[text_set_id].tolist()
    subset = [[x]*4 for x in subset]
    subset = sum(subset, [])
    ...
    for chunk in tqdm(chunk_inputs):
        ...
        for i in range(len(text_inputs)):
            gen_prompt =  random_choice_t2iprompt_from_list() # ' Generate an image based on this description.'
            text_inputs[i] = text_inputs[i] + gen_prompt + '<boi>'
        if cfg_scale>1:
