...
def main(args):
    ...
    paths = ["WISE/cultural_common_sense.json", "WISE/natural_science.json", "WISE/spatio-temporal_reasoning.json"]
    all_data = {}
    for path in paths:
        with open(path, 'r') as f:
            data = json.load(f)
        all_data.update({item["prompt_id"]: item for item in data})

    all_prompts = []
    for idx in range(len(all_data)):
        all_prompts.append({ 'Index':str(idx+1),  'Prompt':all_data[idx+1]["Prompt"]}) 
