...
def main(args):
    ...
    unitok.to('cuda')
    unitok.eval()
    print(f"VQ Model Parameters(inference): {sum(p.numel() for p in unitok.parameters()):,}")
    ...
    # img = torch.stack((img, img), dim=0).to('cuda')  # duplicate the image to form a batch of size 2

    with torch.no_grad():
        code_idx = unitok.img_to_idx(img)
        features = unitok.quantizer.idx_to_f(code_idx)
        print(features.shape)  
        # print(code_idx.shape) # torch.Size([2, 1, 256])
        rec_img = unitok.idx_to_img(code_idx)

    # final_img = torch.cat((img, rec_img), dim=3)
    save_img(rec_img, args.rec_img)
