...
class ImageTextDataset(Dataset):
    ...
    def __init__(self, streaming_dataset, tokenizer_config, rank=0, world_size=1):
        super().__init__()
        self.rank = rank
...
def process_on_gpu(gpu_id, world_size, folder_path, 
    ...
    try:
        ...
        unitok = unitok.to(torch.device(f"cuda:{gpu_id}"))
        unitok.eval()
        dataset = ImageTextDataset(streaming_dataset, tokenizer_config=unitok_cfg, rank=gpu_id, world_size=world_size)
        ...
        for batch in tqdm(dataloader, desc=f"GPU {gpu_id}"):
            if batch is None:
                continue
            tokenized_samples = tokenize_batch(batch, unitok, gpu_id)
...
# python tokenize_to_jsonl_laion_unitok.py --ckpt_path '/path/to/data/unitok_checkpoint_large_clip_1codebook/ckpt-last.pth' \
# --folder_path /path/to/data/filtered_laion_aesthetics_parquet_recaptioned/val.parquet --output_dir /path/to/data/laion_val_recap_tokenized_unitok