...
from misc import init_distributed_mode, random_crop_arr, center_crop_arr

from misc import get_int_prefix_value
...
from misc import LARS
from misc import NativeScalerWithGradNormCount as NativeScaler
from misc import adjust_learning_rate

def load_unitok(ckpt_path, is_gumbel=False):
...
def evaluate(data_loader, vq_model, probe, device):
        ...
        with torch.cuda.amp.autocast():
            with torch.no_grad():
                if args.from_decoder:
                    raise NotImplementedError("from decoder evaluation is not implemented yet")
                    ...
                    indices = vq_model.img_to_idx(images)
                    features = vq_model.quantizer.idx_to_f(indices)
                    features = features.permute(0, 2, 1)  # B, C, L -> B, L, C
                    features = features.mean(dim=-1)  # B, L, C -> B, C
...
def main(args):
    ...
    vq_model, vq_cfg = load_unitok(args.vq_ckpt)
    ...
    transform_train = transforms.Compose([
        transforms.Resize(int(vq_cfg.img_size * vq_cfg.resize_ratio)),
        transforms.CenterCrop(vq_cfg.img_size),
    ...
    if args.eval:
        ...
        corect_pred_num, totoal_num = evaluate(data_loader_val, vq_model, probe, device)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y in loader_iter:
            ...
            with torch.cuda.amp.autocast(dtype=ptdtype): 
                ...
                # print(imgs.shape)
                    if args.from_decoder:
                        raise NotImplementedError("from decoder training is not implemented yet")
                        # quant, diff, spatial = vq_model.encode(
                        #                             imgs, 
                        #                             return_feat=False, 
                        #                             return_fix_dim_feat=False,
                        ...
                        indices = vq_model.img_to_idx(imgs)
                        features = vq_model.quantizer.idx_to_f(indices)
                        features = features.permute(0, 2, 1)  # B, C, L -> B, L, C
                        features = features.mean(dim=-1)  # B, L, C -> B, C
