...
def evaluate(data_loader, vq_model, gpt, probe, device, causal_type, num_code, n_layer=None):
    ...
    for batch in tqdm(data_loader, desc='Evaluating', disable=dist.get_rank() == 0):
        ...
        with torch.cuda.amp.autocast():
            with torch.no_grad():
            # print(imgs.shape)
                _, _, [_, _, indices] = vq_model.encode(
                                                  x,
                ...
                cond_null = torch.ones_like(y.reshape(-1), device=device) * gpt.num_classes
                c_indices = cond_null
                ...
                inner_feat = gpt(
                                ...
                                idx=z_indices[:,:-1], 
                                targets=z_indices, 
                                ret_inner_feat_layer=gpt.n_layer // 2 if n_layer is None else n_layer
                                )
                assert inner_feat.ndim == 3, f"inner_feat must be (B, L, D), current: {inner_feat.shape}"
                # average
                inner_feat = inner_feat.mean(dim=1, keepdim=False)
            outputs = probe(inner_feat)
        
        # calculate the true number
        correct_preds_num += torch.sum(outputs.argmax(dim=1) == y)
...
def main(args):
    ...
    dynamic_length_train = tok_config["model"]["causal_settings"]["dynamic_length_train"]
    min_level = tok_config["model"]["causal_settings"]["min_level"]
    dynamic_level_range = tok_config["model"]["causal_settings"].get("dynamic_level_range", None)
    ...
    vq_model = load_model_from_config(tok_config)
    ...
    if tok_config["model"]["model_cls"] == "VQVitModelPlus":
        is_2d = False
    elif tok_config["model"]["model_cls"] == "VQVitModel":
        is_2d = False
    elif tok_config["model"]["model_cls"] == "VQVitModel2DPlus":
        is_2d = True
    else:
    ...
    dataset_train = ImageFolder(os.path.join(args.data_root, 'train'), transform=transform_train)
    dataset_val = ImageFolder(os.path.join(args.data_root, 'val'), transform=transform_val)
    print(dataset_train)
    print(dataset_val)
    ...
    # load linear probe
    probe = LinProb(in_dim=gpt_model.config.dim).to(device)
    ...
    if args.eval:
        ...
        corect_pred_num, totoal_num = evaluate(
                                        data_loader_val, 
                                        vq_model, 
                                        gpt_model, 
                                        probe, 
        ...
        if rank == 0 and node_rank == 0:
            print(f"LM: Accuracy of the network on the {len(dataset_val)} test images: {accuracy:.4f}")
            # save the number in the experiment folder
            with open(os.path.join(experiment_dir, 'lin_probe_acc.txt'), 'w') as f:
                print(f"LM: Accuracy of the network on the {len(dataset_val)} test images: {accuracy:.4f}", file=f)
        ...
        for x, y in loader_iter:
            ...
            with torch.cuda.amp.autocast(dtype=ptdtype):
                ...
                # print(imgs.shape)
                    ...
                    z_indices = indices.reshape(x.shape[0], -1)
                    cond_null = torch.ones_like(y.reshape(-1), device=device) * gpt_model.num_classes
                    c_indices = cond_null
                    assert z_indices.shape[0] == c_indices.shape[0], f"z:{z_indices.shape}, c:{c_indices.shape}"
                    inner_feat = gpt_model(
                                    cond_idx=c_indices, 
                                    idx=z_indices[:,:-1], 
                                    targets=z_indices, 
                                    ret_inner_feat_layer=gpt_model.n_layer // 2 if args.n_layer is None else args.n_layer
                                    )
                    assert inner_feat.ndim == 3, f"inner_feat must be (B, L, D), current: {inner_feat.shape}"
                    # average
                    inner_feat = inner_feat.mean(dim=1, keepdim=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gpt-ckpt", type=str, default=None, help="gpt ckpt path")
