...
def tokenize_batch(batch, image_tokenizer, gpu_id):
    ...
    try:
        ...
        with torch.no_grad():
            latent, _, [_, _, indices] = image_tokenizer.encode(images)
...
def process_on_gpu(gpu_id, world_size, folder_path, 
    ...
    try:
        ...
        image_tokenizer = image_tokenizer.to(torch.device(f"cuda:{gpu_id}"))
        print(f"GPU {gpu_id}: VQ Model Parameters: {sum(p.numel() for p in image_tokenizer.parameters()):,}")
        ...
        for batch in tqdm(dataloader, desc=f"GPU {gpu_id}"):
            if batch is None:
                continue
            tokenized_samples = tokenize_batch(batch, image_tokenizer, gpu_id)
...
# python tokenize_to_jsonl_laion_gigatok_dino.py --folder_path /path/to/data/filtered_laion_aesthetics_parquet_recaptioned/val.parquet --output_dir /path/to/data/laion_val_tokenized_gigatok_dino