...
def load_and_tokenize_distributed(rank, world_size, folder_path, 
    ...
    try:
        ...
        for batch in tqdm(dataloader, desc=f"Rank {rank}"):
            if batch is None:
                continue
            tokenized_samples = tokenize_batch(batch, image_tokenizer, rank)
...
# python tokenize_to_jsonl_journeydb_gigatok.py --folder_path /path/to/data/predownloaded/datasets/Liquid/journeydb_ingest_all/parquet/validation/ --output_dir /path/to/data/journeydb_val_tokenized_gigatok