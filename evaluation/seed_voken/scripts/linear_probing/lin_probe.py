...
class LinProb(nn.Module):
    def __init__(self, in_dim=256, num_classes=1000):
        super().__init__()
        self.in_dim = in_dim
        ...
        with torch.cuda.amp.autocast():
            with torch.no_grad():
                if args.from_decoder:
                    ...
                    quant, qloss, (_, _, indices) = vq_model.encode(images)
                    # avg poll
                    quant = quant.reshape(quant.shape[0], quant.shape[1], -1).mean(dim=-1)
            outputs = probe(quant)
...
def main(args):
    ...
    vq_model = load_vqgan_new(config, args.vq_ckpt)
            ...
            with torch.cuda.amp.autocast(dtype=ptdtype): 
                ...
                # print(imgs.shape)
                    if args.from_decoder:
                        ...
                        quant, qloss, (_, _, indices) = vq_model.encode(imgs)
