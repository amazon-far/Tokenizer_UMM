...
def process_item(batch,image_tokenizer,source,image_folder):
    ...
    if len(input_images) > 0:
        ...
        with torch.no_grad():
            ...
            vqcode = indices.cpu().view(len(input_images), -1).tolist()
...
def worker_process(rank, args, all_data, world_size):
    ...
    device = f"cuda:{rank}"
    configs = OmegaConf.load("configs/IBQ/gpu/imagenet_ibqgan_1024.yaml")
    encoder = IBQ(**configs.model.init_args)
    sd = torch.load("imagenet256_1024.ckpt", map_location="cpu")["state_dict"]
    ...
    for start_index in tqdm(range(0, len(subset), args.batch_size), desc=f"GPU {rank} processing"):
        ...
        batch_results = process_item(subset[start_index:end_index], encoder, args.source_name, image_folder=args.image_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', type=int, default=256, help='batch size for each process')
    
    args = parser.parse_args()
    main(args)

# python convert_imagepair_cc512_sft_ibq.py --input_pairs /path/to/data/LLaVA-Pretrain/blip_laion_cc_sbu_558k.json --image_path /path/to/data/LLaVA-Pretrain/images/ --temp_path /tmp/llava_pretrain_jsonl --save_path /path/to/data/llava_pretrain_tokenized_ibq_1024  --source_name llava_pretrain
# python convert_imagepair_cc512_sft_ibq.py --input_pairs /path/to/data/playground/dvqa --temp_path /tmp/dvqa_jsonl --save_path /path/to/data/dvqa_tokenized_ibq_1024  --source_name dvqa
# python convert_imagepair_cc512_sft_ibq.py --input_pairs /path/to/data/playground/ai2d --temp_path /tmp/ai2d_jsonl --save_path /path/to/data/ai2d_tokenized_ibq_1024  --source_name ai2d
# python convert_imagepair_cc512_sft_ibq.py --input_pairs /path/to/data/playground/ALLaVA-4V/allava_laion/ALLaVA-Caption-LAION-4V.json --image_path /path/to/data/playground/ALLaVA-4V/  --temp_path /tmp/allava_laion_caption_jsonl --save_path /path/to/data/allava_laion_caption_tokenized_ibq_1024  --source_name allava_laion_caption
# python convert_imagepair_cc512_sft_ibq.py --input_pairs /path/to/data/playground/ALLaVA-4V/allava_vflan/ALLaVA-Caption-VFLAN-4V.json --image_path /path/to/data/playground/ALLaVA-4V/allava_vflan/  --temp_path /tmp/allava_vflan_caption_jsonl --save_path /path/to/data/allava_vflan_caption_tokenized_ibq_1024  --source_name allava_vflan_caption
# python convert_imagepair_cc512_sft_ibq.py --input_pairs /path/to/data/playground/mgm_instruction_noai2d_nodvqa.json --image_path /path/to/data/playground/  --temp_path /tmp/mgm_instruction_jsonl --save_path /path/to/data/mgm_instruction_tokenized_ibq_1024  --source_name mgm_instruction